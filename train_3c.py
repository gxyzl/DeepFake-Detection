        ...
        for i, (img, label) in enumerate(eval_process):
            if i > 0:
                eval_process.set_description( "Epoch: %d, Loss: %.4f, Acc: %.4f" %
                                              (epoch, losses.avg.item(), accuracies.avg))
...
def train_model(model, criterion, optimizer, epoch):
    ...
    for i, (XI, label) in enumerate(training_process):
        if i > 0:
            training_process.set_description("Epoch: %d, Loss: %.4f, Acc: %.4f" % (epoch, losses.avg.item(), accuracies.avg))
...
if __name__ == '__main__':
    root_path = './data_preparation/data_structure/Celeb-DF-v2-face'
    batch_size = 8
    test_batch_size = 64
    input_size = 300
    epoch_start = 0
    ...
    model_path = None
    model_path = './output/weights/efficientnet-b3_3c_LS_Celeb-DF_300_adv_blending/100_acc0.9981.pth'
    # Load model
